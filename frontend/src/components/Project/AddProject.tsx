...
export const AddProject = () => {
  ...
  const { data: projects, isLoading: loadProject } = useQuery({
    queryKey: ["projects", user?.id],
    queryFn: async () => await showProjects(user?.id),
  });
  ...
  return (
    <div className="h-screen overflow-y-hidden w-full relative flex justify-center items-center pt-[4%] bg-primary1">
      ...
      <div className="w-[92%] h-[90%] bg-primary rounded-[52px] p-[48px]">
        ...
        {loadProject ? (
          ...
          <div className="mt-[30px] grid grid-cols-4 auto-rows-max h-[472px] gap-7 overflow-y-auto px-[20px]">
            {project.map((data) => (
              <button
                onClick={() => navigate(`/project/${data.id}`)}
                key={data.id}
                style={{ backgroundColor: `${data.color}` }}
                className="w-full rounded-lg h-[120px] flex items-center justify-center text-[22px] font-bold text-secondary"
              >
                {data.projectname}
              </button>
            ))}
