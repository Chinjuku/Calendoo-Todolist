...
const ProjectTasks = () => {
  ...
  if ( board.boardname === "" && board.boardId === "" &&board.isStarred === false ) {
    const fetchData = async () => {
    ...
    <div className="h-screen w-full relative flex justify-center items-center pt-[4%] bg-primary1">
      ...
      <div className="w-[92%] h-[90%] bg-primary rounded-[52px] p-[26px] flex relative">
        ...
        <div className="flex flex-col justify-center w-[85%] overflow-x-auto overflow-y-hidden px-5">
          {board.boardname === "" &&
          board.boardId === "" &&
          board.isStarred === false ? null : (
            <>
              <h1 className="flex Second text-[39px] w-full pt-5 px-6">
                <p className="">{board.boardname}</p>
                <label className="container">
                  <input type="checkbox" checked={board.isStarred} onChange={handleStar} />
